...
internal static class SuperTrendModuleAppVerification
{
    public static async Task RunAsync(CandleTimeframe timeframe)
    {
        string directory = Path.Combine(
            Path.GetTempPath(),
            "chartkit-app-supertrend-self-test-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            using (var controller = new ChartModulePlatformController(profilePath))
            {
                ...
                ChartUiCommandItem toggle = controller.BuildUiCatalog()
                    .ContextMenuItems.Single(static item =>
                        ...
                        item.Owner.ModuleId == SuperTrendModule.Definition.ModuleId);
                ...
                AssertPlan(controller.RenderPlan, primary, 10, 3f, "default");
                ...
                if (!propertyIds.SequenceEqual(expected))
                    throw new InvalidOperationException(
                        "App SuperTrend property projection failed.");
                ...
                await RequireChangeAsync(
                    controller,
                    toggle.Owner.InstanceId,
                    SuperTrendModule.DownObjectId + ".stroke",
            ...
            using (var restored = new ChartModulePlatformController(profilePath))
            {
                ...
                if (!toggle.IsChecked)
                    throw new InvalidOperationException(
                        "App SuperTrend enabled state was not restored.");
            ...
            Console.WriteLine("csharp_app_supertrend_module_data=PASS");
            Console.WriteLine("csharp_app_supertrend_module_parameters=PASS");
            Console.WriteLine("csharp_app_supertrend_module_style=PASS");
            Console.WriteLine("csharp_app_supertrend_panel_contract=PASS");
            Console.WriteLine("csharp_app_supertrend_module_roundtrip=PASS");
        }
        finally
        {
        ...
        ChartChangeImpact expectedImpact)
    {
        ...
        {
            throw new InvalidOperationException(
                $"App SuperTrend property mutation failed: {propertyId}.");
        }
    }
    ...
    private static ChartPrimarySeriesSnapshot CreatePrimarySeries()
    {
        ...
        {
            double close = 100d + Math.Sin(index / 8d) * 22d + index * 0.03d;
            bars[index] = new ChartPrimaryBar(
                index,
                previous,
                Math.Max(previous, close) + 2.5d,
                Math.Min(previous, close) - 2.5d,
                close,
                4_000L + index * 25L,
                true);
            previous = close;
        }
        return new ChartPrimarySeriesSnapshot(400, bars);
    }
        ...
        string? expectedDownStroke = null)
    {
        Dictionary<string, RenderPrimitivePlan> series = plan.Primitives
            .Where(static item =>
                item.Identity.ModuleId == SuperTrendModule.Definition.ModuleId)
            .ToDictionary(
                static item => item.Identity.ObjectId,
                StringComparer.Ordinal);
        if (series.Count != 2)
            throw new InvalidOperationException(
                $"App SuperTrend {context} primitive count mismatch.");

        RenderPrimitivePlan up = series[SuperTrendModule.UpObjectId];
        RenderPrimitivePlan down = series[SuperTrendModule.DownObjectId];
        if (up.PanelId != "price.main" || down.PanelId != "price.main")
            throw new InvalidOperationException(
                $"App SuperTrend {context} panel mismatch.");

        IReadOnlyList<IndicatorPoint> expected =
            new SuperTrendIndicator(period, multiplier).Calculate(ToCandles(primary));
        bool hasUp = false;
        bool hasDown = false;
